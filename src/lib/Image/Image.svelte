...
<script>
	...
	function setupObservers() {
		...
		const rootObserver = new IntersectionObserver(
			([entry], observer) => {
				const isIntersectingWithRoot = entry.isIntersecting && entry.intersectionRatio >= LIMIT;
		...
		const viewportObserver = new IntersectionObserver(
			([entry], observer) => {
				const isIntersectingWithViewport = entry.isIntersecting && entry.intersectionRatio >= LIMIT;
...
<div class="flex items-center justify-center w-full" style="min-height: 100px">
	<div class:blur bind:this={container} class="container relative w-full">
		<div class="w-full overflow-hidden">
			...
			<img
				src={placeholderSrc}
				{alt}
				class="{className} {loaded ? 'opacity-0' : 'opacity-100'}"
				on:click
			/>
